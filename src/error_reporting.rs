...
/// Enhanced error type for Lut language
#[derive(Debug)]
pub enum LutError {
    ...
    fn from(error: String) -> Self {
        ...
        if error.contains("line") {
            ...
            if let Some(idx) = error.find("line") {
                ...
                if let Some(end_idx) = after_line.find(|c: char| !c.is_digit(10) && c != ' ' && c != ':') {
        ...
        if let Some(idx) = error_msg.find(prefix) {
            ...
            let (number_part, rest) = if !suffix.is_empty() {
                if let Some(suffix_idx) = after_prefix.find(suffix) {
                    (&after_prefix[..suffix_idx], &after_prefix[suffix_idx + suffix.len()..])
                } else {
            ...
            if let Ok(line_num) = number_part.trim().parse::<usize>() {
                ...
                let message = if rest.is_empty() {
                    ...
                    rest.trim_start_matches(|c: char| c == ':' || c == ' ').to_string()
                };
...
pub fn print_error_with_context(error: &LutError, source_code: &str) {
    ...
    if let Some(loc) = location {
        eprintln!("\x1b[1;31m{}\x1b[0m at \x1b[1;33m{}\x1b[0m:", error_type, loc);
    } else {
        ...
        if line_idx < lines.len() {
            ...
            for i in start_line..end_line {
                if i == line_idx {
                    ...
                    eprintln!("\x1b[33m{:>4}\x1b[0m \x1b[31m>\x1b[0m \x1b[1m{}\x1b[0m", i + 1, lines[i]);
                    
                    // Print an error pointer if we have column info
                    if let Some(col) = loc.column {
                        let spaces = " ".repeat(6 + col);
                        eprintln!("{}\x1b[1;31m^\x1b[0m\x1b[31m-- Error occurs here\x1b[0m", spaces);
                    } else {
                        ...
                        if let Some(error_term) = message_lines.first().and_then(|msg| {
                            ...
                            if msg.contains("variable") && msg.contains(':') {
                                ...
                                let end = msg[start..].find('\'').map(|i| i + start).unwrap_or(msg.len());
                                Some(&msg[start..end])
                            } else {
                                None
                            }
                            ...
                            if let Some(pos) = lines[i].find(error_term) {
                                ...
                                eprintln!("      \x1b[1;31m^\x1b[0m\x1b[31m-- Error occurs here\x1b[0m");
                            }
                        } else {
                            eprintln!("      \x1b[1;31m^\x1b[0m\x1b[31m-- Error occurs here\x1b[0m");
                        }
